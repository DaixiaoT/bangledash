...
struct	icmp	{			/* ICMP packet			*/
	...
	union	{
		...
		U32	ic2_gw;		/* for redirect, gateway	*/
		struct {
...
/*
 * Function: send an ICMP message
 */
int icmp_send(U16 type, U16 code, ip_addr_t *dst, void *pa);

/*
 * Function: 处理网络发来的icmp包
 */
int icmp_in(packet_t *pk, ip_hd_t *pip);
